...
/* Email de connexion par lien magique (utilisé par le Send Email Hook,
   voir routes/authHook.js — remplace l'envoi natif de Supabase pour
   pouvoir passer par notre propre lib/email.js, avec son mode test
   EMAIL_TEST_REDIRECT_TO déjà en place pour le partage de sauvegardes). */
function magicLinkEmail({ confirmationUrl }) {
  const text = [
    'Bonjour,',
    '',
    'Voici ton lien de connexion à Traceur. Il est valable une heure et ne peut être utilisé qu\'une seule fois.',
    '',
    ...
    <tr>
      <td align="center" style="padding:32px 16px;">
        <table role="presentation" width="600" cellpadding="0" cellspacing="0" border="0" style="max-width:600px; width:100%; background-color:${COLORS.paper}; border-radius:14px; overflow:hidden;">
          ...
          <tr>
            <td style="padding:8px 32px 28px; font-family:Arial,Helvetica,sans-serif; font-size:15px; line-height:1.6; color:${COLORS.ink};">
              Voici ton lien de connexion à Traceur. Il est valable une heure et ne peut être utilisé qu'une seule fois.
            </td>
          </tr>
          <tr>
            <td align="center" style="padding:0 32px 30px;">
              <table role="presentation" cellpadding="0" cellspacing="0" border="0">
                <tr>
                  <td bgcolor="${COLORS.amber}" style="border-radius:8px;">
                    <a href="${confirmationUrl}" style="display:inline-block; padding:13px 26px; font-family:Arial,Helvetica,sans-serif; font-size:14px; font-weight:700; color:#ffffff; text-decoration:none;">Se connecter à Traceur</a>
                  </td>
                </tr>
              </table>
            </td>
          ...
          <tr>
            <td style="padding:0 32px 28px; font-family:Arial,Helvetica,sans-serif; font-size:12.5px; line-height:1.6; color:${COLORS.inkSoft};">
              Si le bouton ne fonctionne pas, copie-colle ce lien dans ton navigateur :<br>
              <a href="${confirmationUrl}" style="color:${COLORS.inkSoft}; word-break:break-all;">${confirmationUrl}</a>
            </td>
          </tr>
          <tr>
            <td bgcolor="${COLORS.paperDeep}" style="padding:18px 32px; font-family:Arial,Helvetica,sans-serif; font-size:12px; line-height:1.6; color:${COLORS.inkSoft}; border-top:1px solid ${COLORS.line};">
              Si tu n'es pas à l'origine de cette demande, tu peux ignorer cet email sans risque.<br><br>
              Bonne journée,<br>
              <strong>Traceur Team</strong>
            </td>
          </tr>
...
/* Email envoyé à l'éditeur (cedkite@gmail.com) quand un utilisateur
   connecté utilise le formulaire "Nous contacter". */
function contactMessageEmail({ senderEmail, senderName, message }) {
  const text = [
    'Nouveau message depuis le formulaire de contact de Traceur.',
    '',
      ...
      <td align="center" style="padding:32px 16px;">
        <table role="presentation" width="600" cellpadding="0" cellspacing="0" border="0" style="max-width:600px; width:100%; background-color:${COLORS.paper}; border-radius:14px; overflow:hidden;">
          <tr>
            <td bgcolor="${COLORS.headerBg}" style="padding:26px 32px;">
              <span style="font-family:Georgia,'Times New Roman',serif; font-size:24px; font-weight:700; color:${COLORS.gold};">&#127757; Traceur</span><br>
              <span style="font-family:Arial,Helvetica,sans-serif; font-size:11px; letter-spacing:1px; text-transform:uppercase; color:#C9D6C9;">Formulaire de contact</span>
            </td>
          </tr>
          <tr>
            <td style="padding:32px 32px 8px; font-family:Arial,Helvetica,sans-serif; font-size:15px; line-height:1.6; color:${COLORS.ink};">
              Nouveau message depuis le formulaire de contact de l'application.
            </td>
          </tr>
          <tr>
            <td style="padding:8px 32px 20px; font-family:Arial,Helvetica,sans-serif; font-size:13.5px; line-height:1.6; color:${COLORS.inkSoft};">
              De : <strong style="color:${COLORS.ink};">${escapeHtml(senderName)}</strong> &lt;${escapeHtml(senderEmail)}&gt;
            </td>
          </tr>
          <tr>
            <td style="padding:0 32px 30px;">
              <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" bgcolor="${COLORS.paperDeep}" style="border-radius:6px;">
                <tr>
                  <td style="padding:16px 18px; border-left:3px solid ${COLORS.gold}; font-family:Arial,Helvetica,sans-serif; font-size:14px; line-height:1.6; color:${COLORS.ink}; white-space:pre-wrap;">${escapeHtml(message)}</td>
                </tr>
              </table>
            </td>
          </tr>
...
module.exports = { shareRouteEmail, magicLinkEmail, contactMessageEmail };
